...
import static Presentacion.Ordenservicio.cempleado;
import static Presentacion.Ordenservicio.direccion;
import static Presentacion.Ordenservicio.estado;
import static Presentacion.Ordenservicio.idvehiculos;
import static Presentacion.Ordenservicio.marca;
import static Presentacion.Ordenservicio.matricula;
import static Presentacion.Ordenservicio.modelo;
//import static Presentacion.Ordenservicio.inicioalumno;
import static Presentacion.Ordenservicio.nitcliente;
import static Presentacion.Ordenservicio.nombre;
import static Presentacion.Ordenservicio.telefono;
import static Presentacion.Ordenservicio.vehiculo;
 ...
 * @author GLARA
 */
public class BuscarVehiculo extends javax.swing.JInternalFrame {

    //private static Horario frmHorario = new Horario();
    /*El modelo se define en : Jtable-->propiedades-->model--> <User Code> */
    DefaultTableModel model;
    DefaultComboBoxModel modelCombo;
    String[] titulos = {"Matricula", "Modelo", "Marca", "Id"};//Titulos para Jtabla
...
//        Utilidades.setEditableTexto(this.JPanelCampos, false, null, true, "");
    ...
    private void MostrarDatos(String Dato) {
        String[] campos = {"vehiculos.matricula", "vehiculos.modelo", "vehiculos.idmarca", "vehiculos.idvehiculos"};
        String[] condiciones = {"vehiculos.estado=1 and vehiculos.matricula"};
        String[] Id = {Dato};

        if (this.rbCodigo.isSelected()) {
            if (!Dato.isEmpty()) {
                removejtable();
                model = peticiones.getRegistroPorPks(model, "vehiculos", campos, condiciones, Id, "");
            } else {
        ...
        if (this.rbNombre.isSelected()) {
            removejtable();
            model = peticiones.getRegistroPorLike(model, "vehiculos", campos, "vehiculos.estado=1 and vehiculos.modelo", Dato, "");
        }
//        if (this.rbApellido.isSelected()) {
//            removejtable();
//            model = peticiones.getRegistroPorLike(model, "cliente", campos, "cliente.estado=1 and cliente.apellidos", Dato, "");
//        }
...
//    }
    ...
    private void initComponents() {
        ...
        clientes.setModel(model = new DefaultTableModel(null, titulos)
            ...
            jLabel7.setBounds(190, 2, 100, 40);
    ...
    private void clientesKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_clientesKeyPressed
        ...
        if (key == java.awt.event.KeyEvent.VK_ENTER) {
            int p = clientes.getSelectedRow();
            
            matricula.setText(clientes.getValueAt(p, 0).toString());
            vehiculo.setText(clientes.getValueAt(p, 0).toString());
            
            modelo.setText(clientes.getValueAt(p, 1).toString());
            marca.setText(clientes.getValueAt(p, 2).toString());
            //filaseleccionada(alumnos.getValueAt(p, 0).toString());
            //cempleado.removeAllItems();
            //Ordenservicio.llenarcombogrupo(clientes.getValueAt(p, 0).toString());
           // nombrecliente.setText(clientes.getValueAt(p, 1).toString() + " " + clientes.getValueAt(p, 2).toString());
            ...
            //telefonocliente.setText(clientes.getValueAt(p, 3).toString());
           // direccioncliente.setText(clientes.getValueAt(p, 4).toString());
            
            
//            if (clientes.getValueAt(p, 4).toString().equals("Inactivo")) {
//                estado.setText(clientes.getValueAt(p, 4).toString());
//                //estado.setText("Inactivo");
//                estado.setForeground(Color.red);
//            } else if (clientes.getValueAt(p, 4).toString().equals("Activo")) {
//                estado.setText(clientes.getValueAt(p, 4).toString());
//                estado.setForeground(Color.WHITE);
//            }
            idvehiculos=(clientes.getValueAt(p, 3).toString());
            this.dispose();
        }
